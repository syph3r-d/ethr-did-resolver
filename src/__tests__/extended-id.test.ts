...
describe('interpretIdentifier', () => {
  ...
  it('parses did:moon with address', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:moon with address and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:moon with address and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:0x1:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('0x1')
  })
  it('parses did:moon with address and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:rsk:testnet:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  it('parses did:moon with address and sub-network and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:rsk:testnet:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  ...
  it('parses did:moon with publicKey', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:moon with publicKey and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:moon with publicKey and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:mainnet:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('mainnet')
  })
  it('parses did:moon with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:not:so:main:net:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
  it('parses did:moon with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:moon:not:so:main:net:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
